...
const CLOUDINARY_CLOUD_NAME = "dt3rgieaf";
const CLOUDINARY_UPLOAD_PRESET = "Delice_cake_preset";
...
const DEFAULT_SITE_SETTINGS = {
    ...
    ctaText: "Commander",
    telegramChatIds: ["7687122420", "1870863898"]
};

// Service de données abstrait
const DataService = {
    getProducts: async () => {
    ...
    login: async (email, password) => {
        if (isFirebaseConfigured && auth) {
            ...
            if (email.includes('admin')) return { user: { email } };
    ...
    getUserProfile: async (uid, email = null) => {
        ...
        return { role: 'superadmin', permissions: { all: true } };
    },
    ...
    saveOrder: async (orderData) => {
        if (isFirebaseConfigured && db) {
            ...
            orders.push({ ...orderData, id: newId, createdAt: new Date().toISOString() });
    ...
    updateOrderPushToken: async (orderId, token) => {
        if (isFirebaseConfigured && db) {
            try {
    ...
    getSiteSettingsSync: () => DEFAULT_SITE_SETTINGS,
